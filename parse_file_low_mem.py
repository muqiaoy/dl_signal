...
for i in range(len(train_data)):
    ...
    for p in range(int((len(X)-window_size)/stride/k)):
        ...
        np.save("~/low_mem/music_train_x_128_{}.npy".format(index), Xtrain)
        np.save("~/low_mem/music_train_y_128_{}.npy".format(index), Ytrain)
        index = index + 1
...
for i in range(len(test_data)):
    ...
    for p in range(int((len(X)-window_size)/stride_test/k_test)):
        ...
        np.save("~/low_mem/music_test_x_128_{}.npy".format(index), Xtest)
        np.save("~/low_mem/music_test_y_128_{}.npy".format(index), Ytest)
        index = index + 1
